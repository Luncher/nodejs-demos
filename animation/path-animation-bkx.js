...
PathAnimation.prototype.drawTrack = function(ctx) {
	...
	for(var i = 0, len = this.__paths.length; i < len; i++) {
		...
		switch(path.type) {
			...
			case PathAnimation.TYPE_ARC: {
				ctx.arc(path.x, path.y, path.r, path.sAngle, path.eAngle, path.clockwise);
				break;
			}
...
PathAnimation.prototype.arc = function(x, y, radius, sAngle, eAngle, clockwise, duration, interpolator) {
	var path = {
		x: x,
		y: y,
		sAngle: sAngle,
...
PathAnimation.prototype.__getPosition = function(path, prevPos, percent) {
	switch(path.type) {
		...
		case PathAnimation.TYPE_ARC: {
			return this.__arcPosition.apply(this, arguments);
		}
		case PathAnimation.TYPE_ARCTO: {
...
PathAnimation.prototype.__getDirection = function(path, prevPos, easePercent) {
	switch(path.type) {
		...
		case PathAnimation.TYPE_ARC: {
			return this.__arcDirection.apply(this, arguments);
		}
		case PathAnimation.TYPE_ARCTO: {
...
PathAnimation.Math = {
	 ...
	 *计算抛物线长度
	 */
	calcParabolaLen: function(x1, y1, x2, y2) {
		return ;
	}
}
