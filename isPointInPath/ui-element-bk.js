...
UIElement.prototype.openScene = function(name, initData) {
	var me = this;
	setTimeout(function() {
		me.openWindow(name, null, true, initData);
	}, 0);
}
...
UIElement.prototype.isUserMovable = function() {
	...
	if(this.heightAttr === UIElement.HEIGHT_FILL_PARENT 
			&& (this.xAttr === UIElement.X_LEFT_IN_PARENT || this.xAttr === UIElement.X_CENTER_IN_PARENT || this.xAttr === UIElement.X_RIGHT_IN_PARENT)) {
		return false;
	}
...
UIElement.timeScaleIsZero = function() {
	return UIElement.timeScale < 0.00001;
}
...
UIElement.prototype.isPointInMatrix = function(rect, point, applyTransform) {
	...
	applyTransform.apply(matrix);
	
	var p11 = Matrix.transformCoords(matrix, p1.x, p1.y),
		p21 = Matrix.transformCoords(matrix, p2.x, p2.y),
		p31 = Matrix.transformCoords(matrix, p3.x, p3.y),
		p41 = Matrix.transformCoords(matrix, p4.x, p4.y),
		pps = [p11, p21, p31, p41];
	
	return this.pInPolygon(pps, point);
}

UIElement.prototype.isPointInMatrixEx = function(point, x, y, w, h, scaleX, scaleY, rotate) {
	rotate = rotate || 0;
	scaleX = scaleX || 1;
	scaleY = scaleY || 1;
	...
	var points = [p11, p21, p31, p41];
	return this.pInPolygon(points, point.x, point.y);
}
